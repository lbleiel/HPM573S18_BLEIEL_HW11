...
class ParametersFixed():
    def __init__(self, therapy):
        ...
        if therapy == Therapies.NONE:
            self._prob_matrix[:], p = MarkovCls.continuous_to_discrete(Data.RATE_MATRIX, Data.DELTA_T)
        else:
            self._prob_matrix[:], p = MarkovCls.continuous_to_discrete(Data.RATE_ANTICOAG_MATRIX, Data.DELTA_T)
    ...
    def get_annual_state_cost(self, state):
        if state == HealthStats.DEATH or HealthStats.BACKGROUND_DEATH:
            return 0
        else:
    ...
    for s in HealthStats:
        ...
        if s == HealthStats.POST_STROKE:
            ...
            prob_matrix[s.value][HealthStats.STROKE.value]\
                = Data.RR_STROKE*Data.RATE_MATRIX[s.value][HealthStats.STROKE.value]
            # post-stroke to death
            prob_matrix[s.value][HealthStats.DEATH.value] \
                = Data.RR_STROKE * Data .RR_BLEEDING * Data.RATE_MATRIX[s.value][HealthStats.DEATH.value]
            ...
            prob_matrix[s.value] = Data.RATE_MATRIX[s.value]
